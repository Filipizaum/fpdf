...
*                                                                              *
*@file    fpdm.php                                                             *
*@name    A free(zed) and cool FDF / PDF merge tool implemented in php         *
*@package fpdftk                                                               *
*@version 2.6                                                                  *
*@release Snowstream forever dream (decicated to the XMAS snow outside 15cm!  )*
*@date    2010-12-06                                                           *
*@author  0livier                                                              *
		...
		**/
		...
		//------------------------------------
			...
			if(isset($this->value_entries["$name"])) {
				...
				$this->Error("set_field_value of $type failed as the field $name does not exist");
						...
						if($obj) {
							...
							if(preg_match("/endobj/",$CurLine,$match)) {
								...
								if(($type=='Annot')&&($subtype=="Widget")) {
									...
									if(!array_key_exists("current",$values)) $this->Error("Cannot find current Value (/v)  for annotation object $obj,<br>Something goes wrong during parsing (uncompress your pdf and retry) or your pdf is corrupted.");
									if(!array_key_exists("default",$values)) $this->Error("Cannot find default Value (/DV) for annotation object $obj,<br>Something goes wrong during parsing (uncompress your pdf and retry) or your pdf is corrupted.");
						...
						default:
							...
							if($refs_count>0) {
								...
								if($trailer_table==1) { //should be <<
									...
									if(preg_match("/^\/ID\s*\[\s*<([\dA-F]+)/",$CurLine,$match)) {
										$oid=$match[1];
										$id_def=true;
									...
									if($id_def) {//we are inside the ID definition
										if($id_single_line_def||$id_multi_line_def) {
											...
											if(preg_match("/([\dA-F]+)>.*$/",$CurLine,$match)) {
												$tid=$match[1];
									...
									if(preg_match("/^\/DocChecksum \/([\dA-F]+)/",$CurLine,$match)) {
										$checksum=$match[1];
